...
order confirmation), and persistent ticket control buttons.

Persistent views (survive bot restarts because they use static custom_id
values and are re-registered in `setup_hook`): TicketControlView,
TicketReopenView, OpenTicketPanelView.
"""
...
from bot.ui.modals import ReasonModal, collect_dynamic_fields
    ...
    async def mark_completed(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        await interaction.response.send_message(
            embed=embeds.success_embed(
                f"Order #{order['id']} marked as **completed**. The customer can now leave a review with `/review submit`."
            ),
            ephemeral=False,
        )
